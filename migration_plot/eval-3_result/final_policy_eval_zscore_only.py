# Z-Score만 활용하는 청년정책 평가 시스템 (Sigmoid 제외)
...
class YouthPolicyEvaluationZScoreOnly:
    def __init__(self):
    ...
    def calculate_youth_policy_budget(self, region_name):
        ...
        total_budget = 0
        for category_data in policy_execution.values():
            if isinstance(category_data, dict) and "세부사업" in category_data:
                for policy_data in category_data["세부사업"]:
                    if isinstance(policy_data, dict):
                        budget_str = policy_data.get("예산", "0")
                        if isinstance(budget_str, (int, float)):
                            ...
                            try:
                                ...
                                if "억" in budget_str:
                                    budget_str = budget_str.replace("억", "")
                                    total_budget += float(budget_str) * 100000000
                                elif "만" in budget_str:
                                    budget_str = budget_str.replace("만", "")
                                    total_budget += float(budget_str) * 10000
                                else:
    ...
    def calculate_administrative_intensity(self, region_name):
        """행정적 강도를 계산합니다."""
        youth_pop = self.get_youth_population(region_name)
        total_pop = self.get_total_population(region_name)
        finance_autonomy = self.get_finance_autonomy(region_name)
        youth_budget = self.calculate_youth_policy_budget(region_name)

        # 집중도 지수 계산
        if youth_pop > 0:
            concentration_index = youth_budget / youth_pop
        else:
            concentration_index = 0

        # 행정적 강도 = ln(집중도지수/재정자립도+1)
        if finance_autonomy > 0 and concentration_index > 0:
            admin_intensity = math.log(concentration_index / finance_autonomy + 1)
        else:
            admin_intensity = 0

        # 1인당 정책예산 계산
        budget_per_youth = youth_budget / max(youth_pop, 1)

        return {
            "행정적_강도": admin_intensity,
    ...
    def calculate_strategic_intensity(self, region_name, override_region_type=None):
        """전략적 강도를 계산합니다. (Z-Score Only + CDF 적용 버전)"""
        # 정책 카테고리 정의
        policy_categories = ["일자리", "주거", "교육", "복지", "문화", "참여권리"]

        # 전체 지역의 통계 계산
        all_stats = []
        for rname, rdata in self.policy_data.items():
            region_stats = {
                "지역명": rname,
                "is_metro": self.is_metropolitan_area(rname),
            }
            policy_execution = rdata.get("정책수행", {})

            for category in policy_categories:
                count = len(policy_execution.get(category, {}))
                region_stats[f"{category}_정책수"] = count
        ...
        # 현재 지역 데이터 가져오기
        current_region_data = next(
            (r for r in all_stats if r["지역명"] == region_name), None
        )
        ...
        for category in policy_categories:
            ...
            if std > 0:
                ...
                # 정규분포 CDF 적용으로 0~1 사이 값으로 변환
                raw_score = stats.norm.cdf(z_score)
            elif len(distribution) > 0:
                raw_score = 0.5
            else:
                raw_score = 0.0

            # Sigmoid 적용하지 않음 (Z-Score + CDF Only)
    ...
    def save_results(self, df):
        ...
        # 파일 저장
        metro_csv_file = self.result_dir / "zscore_only_광역_청년정책_종합평가결과.csv"
        basic_csv_file = self.result_dir / "zscore_only_기초_청년정책_종합평가결과.csv"
        ...
        linked_csv_file = (
            self.result_dir / "zscore_only_기초_최종평가결과(광역연계).csv"
        )
    ...
    def create_comprehensive_analysis_plots(self, df):
        """종합 분석 플롯 생성 (policy_migration_analysis.py 스타일)"""
        print("\n📊 종합 분석 플롯 생성 중...")
        ...
        axes[0, 1].set_xlabel("전략적 강도 (CDF 적용)")
        ...
        plt.suptitle(
            "청년정책 평가 종합 분석 (Z-Score + CDF 적용)", fontsize=16, y=0.98
        )
        plt.tight_layout()

        # 저장
        comprehensive_path = self.result_dir / "zscore_only_comprehensive_analysis.png"
        try:
    ...
    def create_policy_effectiveness_analysis(self, df):
        ...
        if existing_cols:
            ...
            axes[0].set_ylabel("평균 점수 (CDF 적용)")
        ...
        if "전략적_강도" in df.columns:
            ...
            axes[2].set_title("CDF 적용 전후 분포 비교")
            axes[2].legend()
            axes[2].grid(True, alpha=0.3)

        plt.suptitle("정책 효과성 및 CDF 적용 효과 분석", fontsize=16, y=1.02)
        plt.tight_layout()

        # 저장
        effectiveness_path = self.result_dir / "zscore_only_policy_effectiveness.png"
        try:
    ...
    def create_visualizations(self, df):
        """Z-Score Only 결과 시각화를 생성합니다."""
        print("\n📊 Z-Score Only 시각화 생성 중...")
        ...
        axes[0, 0].set_title("종합점수 분포 (Z-Score Only)", fontsize=14)
        ...
        axes[1, 1].set_title("상위 10개 지역 (Z-Score Only)", fontsize=14)
        ...
        plt.suptitle("청년정책 평가 결과 (Z-Score Only)", fontsize=16)
        plt.tight_layout()

        # 저장
        plot_path = self.result_dir / "zscore_only_evaluation_plots.png"
        try:
        ...
        # 산점도 저장
        scatter_path = self.result_dir / "zscore_only_scatter_plots.png"
        try:
        ...
        # 6. 정착 유도 스타일 플롯 추가 (policy_migration_analysis.py 스타일)
    ...
    def create_correlation_heatmap(self, df):
        ...
        if len(existing_cols) > 1:
            ...
            plt.title(
                "청년정책 평가 지표 상관관계 (Z-Score + CDF 적용)", fontsize=14, pad=20
            )
            ...
            # 저장
            heatmap_path = self.result_dir / "zscore_only_correlation_heatmap.png"
            try:
            ...
            correlations = [(i, j, corr_matrix[i, j]) for i, j in zip(*indices)]
            correlations.sort(key=lambda x: abs(x[2]), reverse=True)

            print(f"\n🔝 가장 강한 상관관계 Top 5:")
            for i, (row_idx, col_idx, corr_val) in enumerate(correlations[:5]):
                row_name = correlation_data.index[row_idx]
                col_name = correlation_data.columns[col_idx]
                significance = (
                    "***"
                    if abs(corr_val) > 0.7
                    else (
                ...
                print(f"  {i+1}. {row_name} ↔ {col_name}: {corr_val:.3f}{significance}")
        else:
            print("❌ 상관관계 분석을 위한 충분한 데이터가 없습니다.")

    def create_settlement_style_plots(self, df):
        """정착 유도 스타일 플롯 생성 (policy_migration_analysis.py와 동일한 스타일)"""
        print("\n📊 정착 유도 스타일 플롯 생성 중...")
        ...
        if "종합점수" in df.columns and "순이동률_인구대비" in df.columns:
            ...
            plt.suptitle(
                "정책 종합점수 vs 순유입률 (광역 vs 기초 vs 전체)\n(Z-Score + CDF 적용, 순유입률 = 순이동/청년인구 × 100)",
                fontsize=16,
                y=0.98,
            )
            plt.tight_layout()
            ...
            # 저장
            save_path = self.result_dir / "zscore_only_settlement_style_plot.png"
            try:
    ...
    def run_evaluation(self):
        """전체 평가 프로세스를 실행합니다."""
        print("=== Z-Score Only 청년정책 종합 평가 시스템 시작 ===")
...
if __name__ == "__main__":
    evaluator = YouthPolicyEvaluationZScoreOnly()
