# 전국 청년정책 종합 평가 시스템 v3
# 전략적 강도: 엔트로피 지수 + 정책수별 정규화된 값 덧셈 (정책 분야별 균형성과 다양성)
# 행정적 강도: 집중도 지수 / 재정자립도 + 집중도 지수 * 재정자립도
...
class YouthPolicyEvaluationSystemV2:
    ...
    def calculate_administrative_intensity(self, region_name):
        """
        [오류 수정 최종본] 정규화 로직이 포함된 행정적 강도 계산 함수.
        - 변수명 충돌 오류를 해결했습니다.
        - Z-score와 CDF를 사용하여 '노력'과 '기여도' 지수의 영향력을 동등하게 보정합니다.
